...
export default function AppPinch() {
  ...
  useEffect(() => { audio.setVolume(settings.volume); }, [settings.volume, audio]);
  useEffect(() => { audio.setSustain(settings.sustain); }, [settings.sustain, audio]);
  ...
  return (
    <div style={styles.page}>
      <header style={{ textAlign: 'center' }}>
        ...
        <p style={styles.subtitle}>Pinch thumb and index finger to play with both hands</p>
      </header>

      <AudioGate audioStatus={audio.audioStatus} sampleStatus={audio.sampleStatus} onUnlock={audio.unlockAudio} />
      ...
      <KeyboardLegend />
      <p style={styles.footer}>HAND PIANO - two-hand pinch control</p>
    </div>
  );
}

function HandDots({ controls }: { controls: ReturnType<typeof usePinchPianoControl>['controls'] }) {
  return (
    <div style={styles.handLayer}>
      {controls.map((control) => (
        <div key={control.id} style={{ ...styles.handDot, left: `${control.xRatio * 100}%` }}>
          {control.handedness} - {control.note ?? 'no key'} - {control.isPinching ? 'pinch' : 'open'}
        </div>
      ))}
    </div>
  );
